...
/// Ramadan, in season (reached from the moon by Today's header and from
/// Me): the next time that matters (iftar by day, the end of suhoor by
/// night) on the kit's lavender, the fasting switch, the night's water, the
/// month's logging on lime, and after Eid the report of what changed. Free
/// for everyone; nothing on this screen is Qamar+.
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        QPageTitle(title: state.digits(season.name(isAr)), isAr: isAr, back: state.back),
  ...
  Widget build(BuildContext context) {
    ...
    return PastelCard(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            Expanded(child: Text(label, style: QText.body(size: 18, weight: FontWeight.w600, color: QColors.onPastel))),
            PastelGlyph(byDay ? QIcons.sunset : QIcons.moon, size: 36),
          ]),
          const SizedBox(height: 8),
          // A clock reads hours first in both languages.
          HeroNumber(time, color: QColors.onPastel, semanticsLabel: '$label $time'),
          const SizedBox(height: 8),
          Text(isAr ? '$where، $other' : '$where · $other', style: QText.body(size: 15, weight: FontWeight.w500, color: QColors.onPastelSecondary)),
        ],
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            Text(isAr ? 'المية بالليل' : 'Water tonight', style: QText.body(size: 16, weight: FontWeight.w600, color: QColors.ink)),
          ]),
          const SizedBox(height: 8),
          Text(
          ...
          for (final (i, w) in windows.indexed) ...[
            ...
            Padding(
              ...
              child: Row(children: [
                QIcon(open == w ? QIcons.waterFull : QIcons.water, size: 20, color: open == w ? QColors.lavender : QColors.inkTertiary),
                const SizedBox(width: 12),
                Expanded(
                  child: Text(
                ...
                Text('${clock(w.fromMin)}–${clock(w.toMin)}', textDirection: TextDirection.ltr, style: QText.number(size: 13, color: QColors.inkSecondary)),
                const SizedBox(width: 12),
  ...
  Widget build(BuildContext context) {
    ...
    return PastelCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(isAr ? 'الشهر ده' : 'This month', style: QText.body(size: 16, weight: FontWeight.w600, color: QColors.onPastel)),
          ...
          Text(
            ...
            style: QText.body(size: 17, weight: FontWeight.w600, color: QColors.onPastel),
          ),
          const SizedBox(height: 12),
          QBar(value: season.days == 0 ? 0 : logged / season.days, onPastel: true),
          ...
          if (state.showScore) ...[
            ...
            Text(
              ...
              style: QText.body(size: 13, color: QColors.onPastelSecondary),
            ),
          ],
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(isAr ? 'اللي اتغيّر في الشهر' : 'What changed this month', style: QText.display(size: 20, ar: isAr)),
          const SizedBox(height: 8),
          ...
          if (!state.plusActive) ...[
            ...
            QTapArea(
              ...
              builder: (context, pressed) => Row(children: [
                ...
                QIcon(QIcons.forward, size: 18, color: pressed ? QColors.ink : QColors.accentInk),
              ]),
            ),
          ],
        ],
