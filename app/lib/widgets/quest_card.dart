...
class QuestCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // On the kit's mint. The lines get the width they need to stay one line
    // each, so every kind keeps to the slot's 120 points in both languages.
    return PastelCard(
      color: QColors.mint,
      ...
      child: Column(
        ...
        children: [
          ...
          ConstrainedBox(
            ...
            child: Row(
              children: [
                Text(QText.eyebrowText(state.t.nextQuest, ar: isAr), style: QText.eyebrow(ar: isAr, color: QColors.onPastel)),
                ...
                if (state.showScore && q.amount > 0) ...[
                  ...
                  Explainable(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      const SuCoinIcon(size: 14, color: QColors.onPastel),
                      const SizedBox(width: 4),
                      ExplainMark(child: Text('+${state.formatSu(q.amount)}', style: QText.number(size: 13, weight: FontWeight.w600, color: QColors.onPastel))),
                    ]),
                  ),
                ],
                const Spacer(),
                if (q.done)
                  const Padding(
                    ...
                    child: QIcon(QIcons.done, size: 22, color: QColors.onPastel),
                  )
                else
                  ...
                  QTapArea(
                    ...
                    builder: (context, pressed) => Padding(
                      ...
                      child: Text(
                        ...
                        style: QText.body(size: 13, weight: FontWeight.w600, color: pressed ? QColors.onPastelSecondary : QColors.onPastel),
                      ),
                    ),
                  ),
              ],
          ...
          Text(q.kind.title(ar: isAr, iso: state.iso), style: QText.body(size: 15, weight: FontWeight.w600, color: QColors.onPastel)),
          const SizedBox(height: 2),
          Text(
            q.done ? (isAr ? 'خلصت النهارده، واتسجّلت.' : 'Done for today, and logged.') : q.kind.why(ar: isAr),
            style: QText.body(size: 13, color: q.done ? QColors.onPastel : QColors.onPastelSecondary),
          ),
        ],
      ),
    );
